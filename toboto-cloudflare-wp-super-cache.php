...
class CloudflareSupercachePurger {
	 ...
	 * @since 1.0.0
	 */
	private string $plugin_slug = 'toboto-supercache-purger';
	...
	public function __construct() {
		$this->cf_email   = defined( 'CLOUDFLARE_EMAIL' ) ? CLOUDFLARE_EMAIL : get_option( 'toboto_cloudflare_supercache_email', '' );
		$this->cf_api_key = defined( 'CLOUDFLARE_API_KEY' ) ? CLOUDFLARE_API_KEY : get_option( 'toboto_cloudflare_supercache_api_key', '' );
		$this->cf_zone_id = defined( 'CLOUDFLARE_ZONE_ID' ) ? CLOUDFLARE_ZONE_ID : get_option( 'toboto_cloudflare_supercache_zone_id', '' );
	...
	public function register_settings() {
		register_setting(
			...
			'toboto_cloudflare_supercache_email',
			array( 'sanitize_callback' => 'sanitize_text_field' )
		);

		register_setting(
			$this->plugin_slug . '-settings-group',
			'toboto_cloudflare_supercache_api_key',
			array( 'sanitize_callback' => 'sanitize_text_field' )
		);

		register_setting(
			$this->plugin_slug . '-settings-group',
			'toboto_cloudflare_supercache_zone_id',
			array( 'sanitize_callback' => 'sanitize_text_field' )
		);

		add_settings_section(
			'toboto_cloudflare_supercache_main_section',
	...
	public function email_field_callback() {
		$email = get_option( 'toboto_cloudflare_supercache_email', '' );
		echo '<input type="text" id="toboto_cloudflare_supercache_email" name="toboto_cloudflare_supercache_email" value="' . esc_attr( $email ) . '" class="regular-text">';
	...
	public function api_key_field_callback(): void {
		$api_key = get_option( 'toboto_cloudflare_supercache_api_key', '' );
		echo '<input type="password" id="toboto_cloudflare_supercache_api_key" name="toboto_cloudflare_supercache_api_key" value="' . esc_attr( $api_key ) . '" class="regular-text">';
	...
	public function zone_id_field_callback() {
		$zone_id = get_option( 'toboto_cloudflare_supercache_zone_id', '' );
		echo '<input type="text" id="toboto_cloudflare_supercache_zone_id" name="toboto_cloudflare_supercache_zone_id" value="' . esc_attr( $zone_id ) . '" class="regular-text">';
		...
		<div class="wrap">
			...
			<div class="card">
				...
				<form method="post" action="">
					<?php wp_nonce_field( 'toboto_cloudflare_manual_purge', 'toboto_cloudflare_manual_purge_nonce' ); ?>
					<input type="hidden" name="toboto_cloudflare_manual_purge" value="1">
					<button type="submit" class="button button-primary">Purge Cloudflare Cache</button>
				</form>
			</div>

			<?php
			if ( isset( $_POST['toboto_cloudflare_manual_purge'] ) && check_admin_referer( 'toboto_cloudflare_manual_purge', 'toboto_cloudflare_manual_purge_nonce' ) ) {
	...
	public function purge_cloudflare_cache() {
		if ( empty( $this->cf_email ) ) {
			$this->cf_email = get_option( 'toboto_cloudflare_supercache_email', '' );
		}

		if ( empty( $this->cf_api_key ) ) {
			$this->cf_api_key = get_option( 'toboto_cloudflare_supercache_api_key', '' );
		}

		if ( empty( $this->cf_zone_id ) ) {
			$this->cf_zone_id = get_option( 'toboto_cloudflare_supercache_zone_id', '' );
		}
